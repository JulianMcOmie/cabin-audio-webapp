...
export class EQCoordinateUtils {
  ...
  /**
   * Calculate visible frequency range based on zoom level and center
   */
  static calculateZoomRange(
    zoomLevel: number, 
    zoomCenter: number
    ...
    // Calculate new range based on zoom level and center
    const visibleRange = logRange / zoomLevel;
    const centerLog = logMin + (logMax - logMin) * zoomCenter;
    
    const newMinLog = Math.max(logMin, centerLog - visibleRange / 2);
    const newMaxLog = Math.min(logMax, centerLog + visibleRange / 2);
    
    return {
      min: Math.pow(10, newMinLog),
      max: Math.pow(10, newMaxLog)
    };
  }
} 